using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public static class PlayerPrefsKeys
{
    public const string AccessToken = "ACCESS_TOKEN";

    public const string AppearanceHairID = "APPEARANCE_HAIR_ID";
    public const string AppearanceHairColorID = "APPEARANCE_HAIR_COLOR_ID";
    public const string AppearanceSkinColorID = "APPEARANCE_SKIN_COLOR_ID";
    public const string AppearanceTopColorID = "APPEARANCE_TOP_COLOR_ID";
    public const string AppearanceBottomColorID = "APPEARANCE_BOTTOM_COLOR_ID";
    public const string AppearanceShoesColorID = "APPEARANCE_SHOES_COLOR_ID";

    public const string DrumsCharacterID = "DRUMS_CHARACTER_ID";
    public const string BassCharacterID = "BASS_CHARACTER_ID";
}
